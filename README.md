...
- **Pi Agent** can run inside the AI assistant through the local backend. It uses Pi SDK sessions, the selected provider/model, editable system prompt, selected indicator notes/code, and read-only project tools (`read`, `grep`, `find`, `ls`). Enable/disable it from the assistant’s ⚙ panel. Generated indicator code still passes the platform validator before import.
